...
const promise_1 = __importDefault(require("mysql2/promise")); // mysql2/promise로 변경
...
const sessionTime = 3000 * 60 * 60 * 1000; //세션 유효시간(ms 단위)
...
passport_1.default.deserializeUser((user, done) => {
    process.nextTick(() => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const [rows] = yield connection.execute('SELECT username, ip, registration_time FROM user WHERE username = ?', [user.username]);
        //console.log(rows);
        // 사용자 정보가 없으면 에러 처리
...
//passport 로그인 작동 부분
passport_1.default.use(new LocalStrategy((username, 입력한비번, cb) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // MySQL에서 사용자 조회
        const [results] = yield connection.execute('SELECT * FROM user WHERE username = ?', [username]);
        //console.log(results); 
        ...
        // 비밀번호 확인
        if (yield bcrypt_1.default.compare(입력한비번, result.password)) { //bcrypt로 비밀번호 확인
...
const getUserInfo = (connection, username) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // SQL 쿼리 실행: 해당 username의 정보를 가져옴 (비밀번호는 제외)
        const [rows] = yield connection.execute('SELECT username, ip, registration_time FROM user WHERE username = ?', [username]);
        // 조회된 유저가 없을 경우
        if (rows.length === 0) {
            return null;
        }
...
app.post('/register', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        yield connection.execute('INSERT INTO user (username, password, ip, registration_time) VALUES (?, ?, ?, ?)', //user 테이블에 삽입
...
io.on('connection', (socket) => __awaiter(void 0, void 0, void 0, function* () {
   ...
   */
    ...
    socket.on('privateMessage', (data) => __awaiter(void 0, void 0, void 0, function* () {
        const receivingUser = data.receivingUser;
        const sentUser = username;
